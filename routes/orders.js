...
router.get('/', authMiddleware, (req, res) => {
  try {
    ...
    if (status) sql += " WHERE order_status = '" + status + "'";
    const countSql = sql.replace('SELECT *', 'SELECT COUNT(*) as count');
    const row = getAll(countSql)[0]; const total = row ? (row.count || row.total || 0) : 0;
    sql += ' ORDER BY created_at DESC LIMIT ' + Number(limit) + ' OFFSET ' + ((Number(page)-1)*Number(limit));
    const orders = getAll(sql);
    orders.forEach(o => { try { o.items = JSON.parse(o.items); } catch(e) { o.items = []; } });
...
router.get('/stats', authMiddleware, (req, res) => {
  try {
    ...
    const todayRev = getAll("SELECT COALESCE(SUM(total),0) as s FROM orders WHERE date(created_at) = ? AND payment_status='paid'", [today]);
...
router.put('/:id/status', authMiddleware, (req, res) => {
  try {
    ...
    const sets = [];
    if (order_status) sets.push("order_status='"+order_status+"'");
    if (payment_status) sets.push("payment_status='"+payment_status+"'");
    if (tracking_number !== undefined) sets.push("tracking_number='"+tracking_number+"'");
    if (order_status === 'delivered') sets.push('delivered_at=CURRENT_TIMESTAMP');
    sets.push('updated_at=CURRENT_TIMESTAMP');
    run('UPDATE orders SET ' + sets.join(',') + ' WHERE id=' + req.params.id);
