...
router.post('/', authMiddleware, (req, res) => {
  try {
    ...
    const r = run('INSERT INTO reviews (product_id, user_id, rating, comment) VALUES (?,?,?,?)', [product_id, req.user.id, Math.min(5,Math.max(1,rating)), comment||'']);
    ...
    res.status(201).json(getOne('SELECT * FROM reviews WHERE id = ?', [r.lastInsertRowid]));
  } catch (e) { res.status(500).json({ message: e.message }); }
});
